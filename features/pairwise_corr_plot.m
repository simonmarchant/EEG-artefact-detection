...
for i = 1:n_var
    for j = 1:n_var
        ...
        if i == j % do univariate histogram
            ...
            if i == n_var
                barh(bin_centre_x, N(1:end-1), 'FaceColor','none','EdgeColor','k');
                ylim([min(bin_edge_x,[],1),max(bin_edge_x,[],1)]); %this was minmax
                 uni_yaxis_max = max(uni_yaxis_max, max(xlim));
            else
                bar(bin_centre_x, N(1:end-1), 'FaceColor','none','EdgeColor','k');
                xlim([min(bin_edge_x,[],1),max(bin_edge_x,[],1)]); %this was minmax
        ...
        elseif i > j % do bivariate histogram
            ...
            if size(X,1) > 100 % for large datasets, do a colourmap
                imagesc(bin_centre_x, bin_centre_y, N(1:end-1,1:end-1));
            else % for small datasets
                scatter(X(:,j), X(:,i), 'kx') % do scatter instead of colourmap
            end
